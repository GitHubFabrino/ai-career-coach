    ...
    <div className="max-w-2xl mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        ...
        <h2 className="text-3xl font-bold text-white mb-2">
          Analyse ton <span className="gradient-text">CV</span>
        </h2>
        <p className="text-sm" style={{ color: 'var(--text-2)' }}>
          {targetCareer ? `Optimisé pour : ${targetCareer}` : 'Détection des compétences · Score global · Feedback personnalisé'}
        </p>
      </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <div
            ...
            style={{
              ...
              background: isDragging ? 'rgba(245,158,11,0.05)' : file ? 'rgba(16,185,129,0.05)' : 'rgba(255,255,255,0.02)',
            }}
          >
            <input ref={inputRef} type="file" accept=".pdf,.txt" className="hidden" onChange={(e) => e.target.files?.[0] && handleFile(e.target.files[0])} />
            ...
            <AnimatePresence mode="wait">
              {file ? (
                <motion.div key="file" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center gap-3">
                  ...
                  <p className="text-xs" style={{ color: 'var(--text-3)' }}>{(file.size / 1024).toFixed(0)} Ko · Clique pour changer</p>
                </motion.div>
              ) : (
                <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-white mb-1">Dépose ton CV ici</p>
          ...
          {error && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-3 flex items-center gap-2 px-4 py-3 rounded-xl text-sm" style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', color: '#f87171' }}>
              <AlertCircle size={14} />
              {error}
            </motion.div>
          )}
          ...
          <motion.button
            ...
            className="btn-primary mt-4 w-full py-3.5 rounded-xl text-sm flex items-center justify-center gap-2"
            style={{
              ...
              boxShadow: file && !isLoading ? '0 4px 20px rgba(245,158,11,0.3)' : 'none',
            }}
          >
            {isLoading ? <><Loader2 size={15} className="animate-spin" /> Analyse en cours…</> : <><FileText size={15} /> Analyser mon CV</>}
          </motion.button>
        </motion.div>
      )}
      ...
      <AnimatePresence>
        {result && (
          <motion.div key="result" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            >
              <div className="absolute inset-0 pointer-events-none" style={{ background: `radial-gradient(ellipse at 50% 0%, ${scoreBg(result.globalScore).replace('0.08', '0.15')}, transparent 70%)` }} />
              <p className="relative text-xs font-semibold uppercase tracking-widest mb-2" style={{ color: scoreColor(result.globalScore), opacity: 0.8 }}>Score global</p>
              <p className="relative font-black" style={{ fontSize: 72, lineHeight: 1, color: scoreColor(result.globalScore) }}>
                {result.globalScore}
                <span className="text-3xl font-bold">/100</span>
              </p>
              <p className="relative text-sm font-semibold mt-1" style={{ color: scoreColor(result.globalScore) }}>
                {scoreLabel(result.globalScore)}
              </p>
              <p className="relative text-sm mt-4 leading-relaxed max-w-md mx-auto" style={{ color: 'var(--text-2)' }}>
                {result.summary}
              </p>
            </div>

            {/* Sections */}
            <div className="p-5 rounded-2xl space-y-4" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)' }}>
              <h3 className="font-bold text-white text-sm">Analyse par section</h3>
              {result.sections.map((section, i) => (
                <motion.div key={section.label} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06 }}>
                  <div className="flex justify-between items-center mb-1.5">
                    <span className="text-sm font-medium text-white">{section.label}</span>
                    <span className="text-xs font-bold tabular-nums" style={{ color: scoreColor(section.score) }}>{section.score}/100</span>
                  </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-4 rounded-2xl" style={{ background: 'rgba(16,185,129,0.05)', border: '1px solid rgba(16,185,129,0.15)' }}>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h4 className="text-xs font-bold uppercase tracking-wide" style={{ color: '#34d399' }}>Détectées</h4>
                </div>
                <div className="flex flex-wrap gap-1.5">
                  {result.detectedSkills.map((s) => (
                    <span key={s} className="px-2 py-0.5 rounded-lg text-[11px] font-medium" style={{ background: 'rgba(16,185,129,0.1)', color: '#6ee7b7' }}>{s}</span>
                  ))}
                </div>
              </div>
              <div className="p-4 rounded-2xl" style={{ background: 'rgba(239,68,68,0.05)', border: '1px solid rgba(239,68,68,0.15)' }}>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h4 className="text-xs font-bold uppercase tracking-wide" style={{ color: '#f87171' }}>Manquantes</h4>
                </div>
                <div className="flex flex-wrap gap-1.5">
                  {result.missingSkills.map((s) => (
                    <span key={s} className="px-2 py-0.5 rounded-lg text-[11px] font-medium" style={{ background: 'rgba(239,68,68,0.1)', color: '#fca5a5' }}>{s}</span>
                  ))}
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              >
                <Upload size={13} /> Analyser un autre CV
              </button>
              <button
                onClick={onRestart}
                className="w-full py-2.5 rounded-xl text-sm font-medium flex items-center justify-center gap-2 transition-colors"
